...
let isBackground =
    localStorage.getItem('isBackground') == null
        ? true
        : localStorage.getItem('isBackground') === 'true';

const canvas: HTMLCanvasElement = document.querySelector('.o-canvas');
...
let calcVelocityY1 = function (timer) {
    ...
    timer += Math.PI / 2;
    return (
        (((Math.sin(timer) * Math.sin(timer)) / 2) * gameTime.deltaTime) / 16
    );
};
...
arrayOfParticleImgPath.push([
    './assets/img/laserBlueP1.png',
    './assets/img/laserBlueP2.png',
]);
arrayOfParticleImgPath.push([
    './assets/img/laserRedP1.png',
    './assets/img/laserRedP2.png',
]);
...
arrayOfMeteorData.push([
    40,
    40,
    20,
    5,
    10,
    './assets/img/meteorBrownSmall.png',
]);
arrayOfMeteorData.push([
    40,
    40,
    ...
    10,
    10,
    './assets/img/meteorGreySmall.png',
]);
...
function calcOpacity(timer) {
    let opacity = (1 / 100000) * timer * (timer - 500) + 1; // Quadratic function, value from 1 to ~0.4
...
function clearScreen() {
    ...
    if (dev) {
        // draw game board
        c.fillRect(
            gameController.startPoint,
            0,
            gameController.unit * 1600,
            gameController.unit * 900
        );
    }
}
...
function GameController() {
    ...
    this.unit =
        (innerWidth / 1600) * 900 <= innerHeight
            ? innerWidth / 1600
            : innerHeight / 900; // calc game unit
    ...
    this.loadResults = function () {
        for (let i = 0; i < 10; i++) {
            if (localStorage.getItem('score' + i) == null) {
                ...
                this.results.push(
                    JSON.parse(localStorage.getItem('score' + i))
                );
            }
        }
    };
    ...
    this.showSettings = function () {
        ...
        this.backgroundButton.innerText = isBackground
            ? 'Wyłącz tło'
            : 'Włącz tło';
    };
    ...
    this.endGame = function () {
        let result = confirm(
            'Przeglądarka może blokować możliwość zamknięcia strony przez skrypt. Czy chcesz spróbować zamknąć kartę ?'
        );
        if (result) {
            window.close();
        }
...
function Player(x, y, img) {
    ...
    this.shieldImg.src = './assets/img/shield.png';
...
function EnemyController() {
    ...
    this.update = function () {
        ...
        for (let i = 0; i < this.arrayOfMeteors.length; i++) {
            ...
            if (
                (this.arrayOfMeteors[i].y - this.arrayOfMeteors[i].width) *
                    gameController.unit >
                innerHeight
            ) {
        ...
        for (let i = 0; i < this.arrayOfPlayerShots.length; i++) {
            ...
            if (
                (this.arrayOfPlayerShots[i].y +
                    this.arrayOfPlayerShots[i].height) *
                    gameController.unit <
                -10
            ) {
        ...
        for (let i = 0; i < this.arrayOfEnemiesShots.length; i++) {
            ...
            // delete when shot get out of screen
            if (
                this.arrayOfEnemiesShots[i].y * gameController.unit >
                innerHeight + 10
            ) {
    ...
    this.collision = function () {
        // meteors and player
        for (let i = 0; i < this.arrayOfMeteors.length; i++) {
            let distanceV = distance(
                player.x,
                player.y,
                this.arrayOfMeteors[i].x,
                this.arrayOfMeteors[i].y
            );
        ...
        for (let i = 0; i < this.arrayOfEnemies.length; i++) {
            let distanceV = distance(
                player.x,
                player.y,
                this.arrayOfEnemies[i].x,
                this.arrayOfEnemies[i].y
            );
        ...
        for (let i = 0; i < this.arrayOfPowerUps.length; i++) {
            let distanceV = distance(
                this.arrayOfPowerUps[i].x,
                this.arrayOfPowerUps[i].y,
                player.x,
                player.y
            );
        ...
        for (let i = 0; i < this.arrayOfEnemiesShots.length; i++) {
            let distanceV = distance(
                this.arrayOfEnemiesShots[i].x +
                    this.arrayOfEnemiesShots[i].width / 2,
                this.arrayOfEnemiesShots[i].y +
                    this.arrayOfEnemiesShots[i].height,
                player.x,
                player.y
            );
        ...
        for (let i = 0; i < this.arrayOfEnemies.length; i++) {
            for (let j = 0; j < this.arrayOfPlayerShots.length; j++) {
                let distanceV = distance(
                    this.arrayOfPlayerShots[j].x +
                        this.arrayOfPlayerShots[j].width / 2,
                    this.arrayOfPlayerShots[j].y,
                    this.arrayOfEnemies[i].x,
                    this.arrayOfEnemies[i].y
                );
                if (distanceV < this.arrayOfEnemies[i].radius) {
                    ...
                    this.arrayOfEnemies[i].getDamage(
                        this.arrayOfPlayerShots[j].dmg
                    );
        ...
        for (let i = 0; i < this.arrayOfMeteors.length; i++) {
            for (let j = 0; j < this.arrayOfPlayerShots.length; j++) {
                ...
                if (distanceV < this.arrayOfMeteors[i].radius) {
                    ...
                    this.arrayOfMeteors[i].getDamage(
                        this.arrayOfPlayerShots[j].dmg
                    );
                    this.arrayOfPlayerShots.splice(j, 1);
                }
    ...
    this.waveStart = function () {
        ...
        do {
            ...
            arrayOfWaveData[randomWave][0] > gameTime.time &&
            (arrayOfWaveData[randomWave][1] < gameTime.time ||
                arrayOfWaveData[randomWave][1] !== -1)
        );
...
function Meteor(x, y, dx, dy, step, type) {
    ...
    this.img = new Image();
    this.img.src = arrayOfMeteorData[type][5];
...
function Shot(x, y, dy, dmg, type) {
    ...
    this.type = type;
    this.img = new Image();
    this.img.src = arrayOfShotsImgPath[type];
    this.dmg = dmg;
            ...
            for (let j = 0; j < i; j++) {
                ...
                if (
                    distance(
                        x,
                        y,
                        this.arrayOfStars[j].x,
                        this.arrayOfStars[j].y
                    ) <
                    radius + this.arrayOfStars[j].radius
                ) {
                    noCollision = false;
                }
            }
...
gameController.startButton.addEventListener('click', () =>
...
gameController.soundButton.addEventListener('click', function () {
    isSound = !isSound;
    gameController.soundButton.innerText = isSound
        ? 'Wyłącz dźwięk'
        : 'Włącz dźwięk';
    localStorage.setItem('isSound', String(isSound));
});

gameController.backgroundButton.addEventListener('click', function () {
    isBackground = !isBackground;
    gameController.backgroundButton.innerText = isBackground
        ? 'Wyłącz tło'
        : 'Włącz tło';
    localStorage.setItem('isBackground', String(isBackground));
});
